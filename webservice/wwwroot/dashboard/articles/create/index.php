...
if(isset($_POST['commit']) && $_POST['commit'] == 1){
 ...
 header('Location: /dashboard/articles/?notification=Article+Created');
 die();
...
<option value="user" <?php echo isset($_POST['owner']) && $_POST['owner'] == 'user' ? 'selected' : ''?>>Me</option>

<?php foreach($CURRENT_USER->adminGroups() as $group):?>
<option value="<?php echo $group->id()?>" <?php echo isset($_POST['owner']) && $_POST['owner'] == $group->id() ? 'selected' : ''?>><?php echo $group->name() ?></option>
<?php endforeach ?>
</select>
